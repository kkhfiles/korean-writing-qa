...
NO_WORK_WORD = [
    ...
    #   ⛔ ★ 「죽이다」(타동)는 뺀다 — **일부러 끝내는 것은 표준 기술 한국어**다
    #      (「부모만 죽이면 손자가 남는다」·「psutil tree-kill 로 죽인다」).
    #      가르는 것은 조사다 — 「프로세스가 죽다」(은유)와 「프로세스를 죽이다」(동작).
    #      적대 시험에서 우리 운영 문서 두 곳이 걸려 찾았다.
    #      ⚠️ 음절 결합에 **두 번째로** 걸렸다 — 「죽인다」에는 「죽이」가 없다
    #      (이 + ㄴ다 → 인다). 「죽음」은 죽+음이라 그대로 걸린다.
