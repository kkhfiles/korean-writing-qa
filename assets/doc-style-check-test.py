...
<section>
<h2>풀려던 문제</h2>
<p>완료한 업무에 실제로 몇 시간 걸렸는지 적는 칸이 있다. 체크인이 매일 물었지만
열닷새 동안 새로 적힌 것이 하나도 없었다. 사람이 게을러서가 아니라 묻는 시점이
일이 끝나고 몇 시간 뒤라 이미 기억이 흐렸다.</p>
<p>묻는 지점을 완료를 누르는 그 순간으로 옮기고, 안 적힌 완료 업무를 화면 맨 위에
가로로 늘어놓아 하나씩 치우게 했다. 더 자주 묻지 않았고 묻는 지점만 옮겼다.</p>
</section>
""")
