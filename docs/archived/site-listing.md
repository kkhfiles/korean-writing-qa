...
| 무엇 | 정해진 것 |
|---|---|
| **사이트 이름** | **「Dispatch」로 확정**(2026-09-08) · 전에는 「Dispatch & Mycelium」 · 구조화 자료·breadcrumb·`llms.txt` 가 다 이 글자를 씀 · 셋째 도구가 붙으면 낡음 → 바꿀지 · 그대로 둘지 |
| **「두 도구 비교」 표** | 셋으로 늘릴지 · 둘만 두고 교정기는 카드로만 소개할지 |
| **경로** | **`/vermilion/` 그대로 두기로 확정**(2026-09-10 사용자) · 이름이 「AI 한글 교정기」로 바뀐 뒤에도 안 옮김 — 워커에 리다이렉트가 없어 옮기면 옛 링크가 끊김 |
