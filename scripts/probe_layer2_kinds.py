...
**2026-09-17 실측 · sonnet · temperature 0 · 세 벌**

① 대장 17건만 심은 운반 문서(2회) ② 예비 11건을 더한 운반 문서(2회)
③ 확정 교정 셋을 `core-rules.md` 표에 올린 뒤(1회).

| 재는 것 | blind | guided |
|---|---|---|
| 짚은 자리(표현) · 심은 28건 기준 | 12 · 10 · 13 | **15 · 15 · 15** |
| 판정 이름(까닭) · 항목 이름이 적힌 것 기준 | 0 · 0 · 0 | **14 · 12 · 3** |
| 예비 8건(의심 뺀 것) · 표현 | 6 · 4 · 4 | 4 · 5 · 4 |

★ **까닭 기준은 못 쓴다 — 규칙 파일을 고치면 무너진다.** ③에서 표에 세 줄을
더하자 모델이 9단계 항목 이름 대신 **상위 규칙 이름**을 쓰기 시작했다
(「한 낱말로 있는 행위 명사를 동사구로 품」 6건 → 「업무에서 쓰는 말로 쓰기」
5건). 까닭은 14·12 에서 3 으로 떨어졌는데 **짚은 자리는 15 로 그대로다.**
행동이 아니라 이름이 바뀐 것이다. 판정 이름은 정해진 어휘가 아니다.

★ **인용 안 한 같은 갈래에서는 blind 와 차이가 없다** — 예비 8건에서 세 벌
내내 blind 6·4·4 · guided 4·5·4 다. 앞 판에 적었던 「0 → 4」는 까닭 기준으로
센 것이라 **이름이 guided 쪽 규칙에만 있어서 생긴 착시**였다.

**까닭이 달라도 낸 고침이 같다.** blind 가 「막연한 비유 줄이기」로 짚은 자리의
고침이 9단계가 의도한 것과 같았다 — 「자리를 잘못 잡으면」→「시점을 잘못
잡으면」 · 「자기 자리를 찾습니다」→「자동으로 분류됩니다」 · 「같이 보게
두기」→「결과를 공유합니다」. **문서는 같게 고쳐진다.**

- **9단계의 값은 회수율이 아니라 이름을 정확히 붙이는 것일 수 있다.** 되먹임
  (어느 갈래가 몇 건인지 세는 일)에는 쓰이고, 문서를 고치는 데에는 핵심 규칙
  만으로 충분해 보인다. **단정하지 않는다** — 예비 예문이 이 저장소가 지어낸
  것이고, 그 갈래들이 핵심 규칙의 상위 항목으로도 덮이기 때문이다.
- **⚠️ 운반 문서를 늘리자 대장 재현율이 흔들렸다** — 심은 것이 17개에서
  29개로 늘자 회차마다 달라진다. 긴 문서에서 2층이 약해진다는 뜻이므로
  회수율을 문서 길이와 함께 읽어야 한다.
- **③은 한 회차뿐이다.** 칸별 수치(둘 다 인용 7 · guided 만 인용 8 · 둘 다
  비인용 13)는 회차가 하나라 판정 불가다.
- **음성 대조** — 「고치지 않는 검사기입니다」는 결함이 아니라서 예비에서 뺐고
  운반 문서에 남겨 두었다. 세 벌 내내 **아무 갈래도 안 짚었다.**
...
def report(out: dict) -> None:
    ...
    if out["gradable"]:
        ...
        print("      ⚠️ 까닭 기준은 **규칙 파일을 고치면 흔들린다** — 이름이 "
              "정해진 어휘가 아니다.\n"
              "         2026-09-17 에 표 세 줄을 더하자 모델이 9단계 항목 "
              "이름 대신 상위 규칙\n"
              "         이름을 쓰기 시작해 14·12 에서 3 으로 떨어졌다. "
              "짚은 자리는 15 로 그대로였다.")
    else:
        print("      까닭은 안 봤다 — 대장이 항목 이름을 안 적는다 "
              "(note 에 「§9 「항목 이름」」까지 적으면 본다)")
    # ⛔ 위 블록은 `if`/`else` 다. 예전에는 `for`/`else` 라 언제나 찍혔다.
    for tag in ("대장", "예비"):
...
def main() -> None:
    ...
    if len(results) == 2:
        ...
        def spread(out):
            """**표현 기준**으로 센다 — 까닭 기준은 규칙을 고치면 흔들린다.

            까닭이 달라도 **낸 고침이 맞으면 문서는 같게 고쳐진다.** 실측에서
            blind 가 「막연한 비유 줄이기」로 짚은 자리의 고침이 9단계가 의도한
            것과 같았다(「자리를 잘못 잡으면」 → 「시점을 잘못 잡으면」).
            """
            pick = [r for r in out["rows"] if r["source"] == "예비"]
            sure = [r for r in pick if not r.get("doubt")]
            return (sum(1 for r in pick if r["tight"]), len(pick),
                    sum(1 for r in sure if r["tight"]), len(sure))
        ...
        if ng == 0:
            ...
            print(f"   글자 밖으로 번지나 — 예비 {ng}건에서 표현 기준 "
                  f"{gb} → {gg}")
            print(f"      의심 뺀 {mg}건만 — {sb} → {sg}")
            if sg > sb:
                print(f"\n적어 두면 인용 안 한 같은 갈래도 더 잡는다 — "
                      f"{mg}건 중 {sb} 에서 {sg} 로.")
            else:
                print("\n인용 안 한 같은 갈래에서는 **차이가 없다** — 핵심 "
                      "규칙만으로도 같은 자리를 짚는다.\n"
                      "   9단계의 값은 회수율이 아니라 **이름을 정확히 "
                      "붙이는 것**일 수 있다(되먹임에는 쓰이고\n"
                      "   문서 고치기에는 안 쓰인다). 회차마다 뒤집히니 "
                      "한 회차로 단정하지 않는다.")
