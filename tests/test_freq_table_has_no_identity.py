"""공개되는 빈도표에 신원이 없는지, 줄인 것이 훑개 판정을 안 바꾸는지 본다.
...
class FreqTableTests(unittest.TestCase):
    ...
    def test_the_floor_matches_what_the_sweep_asks_for(self) -> None:
        """문턱이 어긋나면 훑개 판정이 조용히 바뀐다.

        훑개는 `freq.get(key, 0) <= --max` 로 판정한다 — 표에 없는 낱말은 0이다.
        ...
        self.assertIsNotNone(m, "훑개의 --max 기본값을 못 찾았습니다")
        default = int(m.group(1))
        self.assertLessEqual(
            scrub.FLOOR, default,
            f"표를 {scrub.FLOOR}회에서 잘랐는데 훑개 기본 문턱은 {default}회입니다 — "
    ...
    def test_single_letters_are_gone(self) -> None:
        """훑개는 한 글자를 안 읽는다 — 성씨와 이름 낱자가 거기 모인다."""
        short = [k for k in table() if len(k.rsplit("/", 1)[0]) < scrub.MIN_LEN]
        self.assertEqual([], short, f"한 글자 항목 {len(short)}종이 남았습니다")

    def test_only_the_tags_the_sweep_reads_are_kept(self) -> None:
        tags = {k.rsplit("/", 1)[1] for k in table()}
        self.assertTrue(tags <= scrub.KEEP_TAGS, f"훑개가 안 읽는 품사: {tags - scrub.KEEP_TAGS}")
