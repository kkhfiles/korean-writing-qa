...
class CoreRulesMarkerTests(unittest.TestCase):
    ...
    def test_the_made_up_noun_rule_keeps_its_test_and_its_carve_out(self) -> None:
        ...
        **처음에 「결정 규칙으로는 못 넣는다」고 접었던 것을 뒤집었다.** 그때 근거는
        세 뭉치 11건이 전부 정상이라는 실측이었는데, 여섯 낱말을 **한 덩어리로**
        센 수치였다. 낱말별로 다시 재니 갈렸다 — 값·축·층이 그 11건을 거의 다
        차지했고, 판·창·결은 실문서 1,161개에서 0건이었다. 그래서 앞 셋만 전역
        검사기로 갔다(`tests/test_empty_noun_rule.py`).

        **뭉뚱그린 실측이 규칙 하나를 통째로 접게 만들었다**는 것이 이 건의 값이다.
        판정 기준과 굳은 말 예시는 그대로 남아야 한다 — 사람이 보는 값·축·층은
        여전히 이 표시가 전부이고, 검사기가 보는 셋도 예외를 이 문장에서 가져온다.
        """
