...
**배포된 스킬 필수** — 탐지 논리의 정본은 `~/.claude/skills/finalize-korean-document`이고 이 저장소의 스캐너는 그것을 불러다 쓴다. 없으면 수집 단계에서 멈추며 찾던 경로를 알려 준다. 다른 위치에 두었다면 `KOREAN_WRITING_SKILL_HOME`으로 지정한다.

**사본을 두지 않는 이유** — 같은 정규식을 두 벌 두면 한쪽만 고쳐진다. 실제로 갈라져서, 스킬을 망가뜨려도 이 저장소 시험은 전부 통과했다. 조용히 사본으로 돌아가는 대신 바로 멈춘다.
