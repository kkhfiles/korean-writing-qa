...
| `install.py --check` | `같음 27 · 다름 0 · 없음 0` (저장소 밖 신원 목록을 둔 기계는 28) |
| 시험 | `OK` · 건너뛰기 2건은 정상 |

**건너뛰기 2건이 정상인 까닭** — 작성자 기계에만 있는 연구 자료(사내 문서 복사본)를 대조하는 시험입니다. 검사기와 스킬을 재는 시험은 전부 돕니다.

**이 표는 시험이 지킵니다** — `tests/test_readme_expected_values.py` 가 README 의 숫자를 읽어 실제로 돌려 봅니다. 예전에는 아무도 안 지켜서 `오류 10 · 주의 5`·`건너뛰기 14건`이 조용히 낡아 있었고, 그대로 따라 한 사람은 **설치가 깨졌다고 읽었을 것입니다**.
...
```
python assets/doc-style-check.py --list-rules      # 갈래 30종 · 묶음 5종
...
- **제품 이름은 저장소 밖 목록에 둠** — `data/catalog/local-brand-names.example.txt` 를 `local-brand-names.txt` 로 복사해 자사 제품 이름과 잘못 적은 꼴을 적습니다. 복사본은 `.gitignore` 대상이라 안 올라갑니다 · 목록이 없으면 그 갈래는 안 돌고 **안 봤다고 출력에 적습니다**
